...
CREATE TABLE IF NOT EXISTS usuarios (
  ...
  email VARCHAR(255) NOT NULL,
  nome VARCHAR(255) NOT NULL,
  papel VARCHAR(50) NOT NULL DEFAULT 'atendente',
...
-- Pontos de Censo (PLAQUETA)
CREATE TABLE IF NOT EXISTS pontos_censo (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  prefeitura_id UUID NOT NULL REFERENCES prefeituras(id) ON DELETE CASCADE,
  plaqueta VARCHAR(50) NOT NULL,
  ...
  historico_versoes INTEGER DEFAULT 1,
  criada_em TIMESTAMP DEFAULT now(),
  atualizada_em TIMESTAMP DEFAULT now(),
  UNIQUE(prefeitura_id, plaqueta)
);

-- Índices para pontos_censo
CREATE INDEX IF NOT EXISTS idx_pontos_census_bairro ON pontos_censo(prefeitura_id, bairro);
CREATE INDEX IF NOT EXISTS idx_pontos_census_status ON pontos_censo(prefeitura_id, status);
...
CREATE INDEX IF NOT EXISTS idx_atendimentos_status ON atendimentos(prefeitura_id, status);
CREATE INDEX IF NOT EXISTS idx_atendimentos_bairro ON atendimentos(prefeitura_id, bairro);
CREATE INDEX IF NOT EXISTS idx_atendimentos_data ON atendimentos(prefeitura_id, data_criacao);
...
CREATE INDEX IF NOT EXISTS idx_despachos_equipe ON despachos(equipe_id, data_despacho);
CREATE INDEX IF NOT EXISTS idx_despachos_status ON despachos(prefeitura_id, status);
...
-- Índices para modernizacoes
CREATE INDEX IF NOT EXISTS idx_modernizacoes_status ON modernizacoes(prefeitura_id, status);
CREATE INDEX IF NOT EXISTS idx_modernizacoes_data ON modernizacoes(prefeitura_id, data_conclusao);
...
CREATE INDEX IF NOT EXISTS idx_historico_tabela ON historico_alteracoes(tabela_afetada, registro_id);
CREATE INDEX IF NOT EXISTS idx_historico_data ON historico_alteracoes(criado_em);

-- Versões de Pontos Censo
CREATE TABLE IF NOT EXISTS versoes_ponto_censo (
  ...
  reservico BOOLEAN DEFAULT false,
  observacoes TEXT,
  criada_em TIMESTAMP DEFAULT now(),
